...
/**
 * Create a live stream of all the Tasks.
 *
 * @return {Stream}
 */

Tasks.prototype.findStream = function () {
  var self = this;
  var tr = through();
  
  process.nextTick(function () {
    self.set.onEach(function (row) {
      tr.write(Task(row, self.set));
    });
  });
...
/**
 * Get the insertion timestamp.
 *
 * @return {Number}
 */

Task.prototype.ts = function () {
  return this._row.get('ts');
}
